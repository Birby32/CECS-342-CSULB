...
void* my_alloc(int size) {
    ...
    double doubleSizeAsMultiple = ceil((double)size / (double) POINTER_SIZE);
    int sizeAsMultiple = (int)(doubleSizeAsMultiple * POINTER_SIZE);
	...
	while (curr != NULL) {
		if (curr->block_size >= sizeAsMultiple) {
			...
			if (excessSpace >= (OVERHEAD_SIZE + POINTER_SIZE)) { // Splittable
				...
				// Update Curr's block size as a result of splitting.
                                //Your code
                struct Block *newBlock = (struct Block*)((char*) curr + OVERHEAD_SIZE + sizeAsMultiple);
                newBlock->block_size = excessSpace;
                newBlock->data = ((char*)newBlock + OVERHEAD_SIZE);

				// Adjust the double linked list, depending on whether curr is the head or not.
                                //Your code
                if(curr == free_head) free_head = newBlock;
                else{
